...
pub struct StockRecord {
    ...
    pub _static_pe_rate: u32,
    pub _static_dynamic_pe: u32,
...
impl StockRecord {
    fn push(&self, ctx: &mut T2Context, func_no: i32) -> io::Result<()> {
        ...
        unsafe {
            ...
            t2message_addfield(msg._message,
                               CString::new("pre_close_price").unwrap().as_ptr());
            ...
            t2message_addfield(msg._message,
                               CString::new("margin_status").unwrap().as_ptr());

            t2message_addstr(msg._message,
                             CString::new(self._stock_code.as_str()).unwrap().as_ptr());
            ...
            t2message_addstr(msg._message,
                             CString::new(self._trade_amount.to_string().as_str())
                                 .unwrap()
                                 .as_ptr());
            t2message_addstr(msg._message,
                             CString::new(self._trade_amount.to_string().as_str())
                                 ...
                                 .as_ptr());
            t2message_addstr(msg._message,
                             CString::new(self._trade_balance.to_string().as_str())
                                 .unwrap()
                                 .as_ptr());
            /*
            t2message_addint(msg._message, self._trade_amount);
            t2message_addint(msg._message, self._trade_amount);
            t2message_addint(msg._message, self._trade_balance); */
            if self._time <= 91500 {
            ...
            t2message_addint(msg._message, self._margin_status as i64); //todo: margin status
...
impl Default for StockRecord {
    fn default() -> StockRecord {
        ...
        StockRecord {
            ...
            _static_pe_rate: 0u32,
            _static_dynamic_pe: 0u32,
            _pe_rate: 0u32,
            _dynamic_pe: 0u32,
...
use log::*;
#[allow(dead_code)]
pub fn push_stock_status(ctx: &mut T2Context,
                         code: &str,
                         trade_status: u32,
                         time: u32)
                         -> io::Result<()> {
...
pub fn push_market_time(ctx: &mut T2Context, date: u32, time: u32) -> io::Result<()> {
    ...
    unsafe {
        ...
        t2message_addfield(msg._message, CString::new("line_no").unwrap().as_ptr());
        t2message_addfield(msg._message,
                           CString::new("market_status").unwrap().as_ptr());
    ...
    push_min_change(ctx, date, time)?;
    Ok(())
}

static mut __PRE_TIME : u32 = 0;
pub fn push_min_change(ctx : &mut T2Context, date : u32, time:u32)->io::Result<()> {
    ...
    unsafe {
        ...
        if (time / 100) > (__PRE_TIME / 100) {
            ret = push_market_status(ctx, date, time, 6);
        }
...
static mut __OPEN_DATE : u32 = 0;
pub fn push_market_open(ctx : &mut T2Context, date : u32, time : u32)->io::Result<()> {
    ...
    if time >= 85900 && time <= 90200 {
        t = time;
    }

    push_market_status(ctx, date, t, 2)
}

static mut __MARKET_STATUS: u32 = 0;
static mut __DATE_INIT: u32 = 0;
static mut __OLD_TIME:u32 = 0;
fn push_market_status(ctx: &mut T2Context,
                          date: u32,
                          time: u32,
                          trade_status: u32)
                          -> io::Result<()> {
    ...
    unsafe {
        ...
        t2message_addfield(msg._message,
                           CString::new("market_status").unwrap().as_ptr());
        t2message_addfield(msg._message,
                           CString::new("market_time_old").unwrap().as_ptr());
